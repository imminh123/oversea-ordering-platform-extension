...
const isTaobao = location.hostname === "item.taobao.com" && !new URL(location.href).searchParams.get('country') === 'GLOBAL';
const isTaobaoGlobal =  location.hostname === "item.taobao.com" && new URL(location.href).searchParams.get('country') === 'GLOBAL';
...
function main() {
  ...
  var varientTaobaoGlobalContainer = Array.from(elements).filter((element) => {
    return Array.from(element.classList).some((className) =>
  ...
  if (varientTaobaoGlobalContainer && isTaobaoGlobal) {
    const customActionsContainer = document.createElement("div");
    customActionsContainer.id = "to-platform";
    customActionsContainer.innerHTML = `
    <img style=" width: 100px;float: right; margin-top: 5px;" src="https://i.ibb.co/CWFcJjc/mby.png" alt="logo_mby"/>
    ...
    varientTaobaoGlobalContainer.append(customActionsContainer);
  }
}
